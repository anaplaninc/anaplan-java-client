...
public class Service implements Closeable {
  ...
  public Iterable<ModuleData> getModules(String workspaceId, String modelId)
    ...
    return new Paginator<ModuleData>() {
      ...
      public ModuleData[] getPage(int offset) {
        ModulesResponse response = apiProvider.get()
            .getModules(workspaceId, modelId, offset);
        setPageInfo(response.getMeta().getPaging());
        if (getPageInfo().getCurrentPageSize() > 0 && response.getItem() != null) {
  ...
  public Iterable<ViewData> getViews(String workspaceId, String modelId, String moduleId)
    ...
    return new Paginator<ViewData>() {
      ...
      public ViewData[] getPage(int offset) {
        ViewsResponse response = apiProvider.get()
            .getViews(modelId, moduleId, offset);
        setPageInfo(response.getMeta().getPaging());
        if (getPageInfo().getCurrentPageSize() > 0 && response.getItem() != null) {
  ...
  public void exportListNames(String fileType, String fileId, String workspaceId, String modelId) {
    ...
    if (workspace != null && modelId != null) {
      List<ListName> listNames = StreamSupport.stream(getListNames(workspace.getId(), modelId).spliterator(), false)
  ...
  public ListMetadata getListMetadata(String workspaceId, String modelId, String listId)
      throws AnaplanAPIException {
    try {
      ListMetadataResponse response = apiProvider.get()
          .getListMetadata(workspaceId, modelId, listId);
   ...
   * @since 2.0
   */
  public Iterable<ListName> getListNames(String workspaceId, String modelId)
      throws AnaplanAPIException {
    try {
