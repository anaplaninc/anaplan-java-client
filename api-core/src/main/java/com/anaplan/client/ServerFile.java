...
public class ServerFile extends NamedObject {
    ...
    try {
      ...
      while (chunkIterator.hasNext()) {
        ...
        if (separatorLastIndex == -1) {
          // if the separator is missing from the import file data we stop the upload
          throw new IllegalStateException(
              "Unable to find data separator " + data.getSeparator() + " in the data file " + source
                  .getName());
        }
   ...
   */
  static List<String> splitSeparators(String separators) {
