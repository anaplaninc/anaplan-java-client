...
# 3. Build the Java binary path using the JAVA_HOME value, validate that 'JAVA_HOME\bin\java -fullversion' is the valid Java supported, which is Java8, Java11 and OpenJDK8.

# Provide the absolute path to Java8 or Java11 or OpenJDK8 installation (optional)
# NOTE: If the customer requires Java_Home to be set to a version other than Java8, Java11 or OpenJDK8, then override this
# section with the directory for Java8, Java11 or OpenJDK8)
_java=
...
if [ -z "$_java" ]; then
  echo "Absolute path to JAVA8 or JAVA11 or OpenJDK8 executable not provided. Will attempt to figure out path from JAVA_HOME or which command..."
...
# Check if we have the correct version of Java (8 or 11 or openJDK8)
if [[ "$_java" ]]; then
  java8=0
  java11=0
  javaOpen=0
  if [ `${_java} -version 2>&1 | grep 'open' | wc -l` -gt 0 ]; then
        echo "OpenJdk found!"
        javaOpen=1
  fi
  ...
  if [ `${_java} -version 2>&1 | grep 8.0 | wc -l` -gt 0 ]; then
      echo "Java 8 found!"
      java8=1
  fi
  if [ `${_java} -version 2>&1 | grep 11.0 | wc -l` -gt 0 ]; then
      echo "Java 11 found!"
      java11=1
  fi
  if [[ ${java11} == 1 && ${javaOpen} == 1 ]]; then
      echo "ERROR: The Java version you have is not supported by Anaplan Connect. Please upgrade to Java8, Java11 or OpenJDK8." >&2
      exit 100
  fi
  if [[ ${java8} == 0 && ${java11} == 0 ]]; then
    echo "ERROR: The Java version you have is not supported by Anaplan Connect. Please upgrade to Java8, Java11 or OpenJDK8." >&2
    exit 100
  fi
fi
...
classpath=`ls -t "${here}"/anaplan-connect-*-jar-with-dependencies.jar | head -1`
if [ ! -f "${classpath}" ]; then
