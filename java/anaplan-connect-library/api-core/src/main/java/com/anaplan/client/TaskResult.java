  ...
  public ServerFile getFailureDump() {
    ...
    return new ServerFile(task.getSubject().getModel(), serverFileData) {
      ...
      public List<ChunkData> getChunks() {
        if (isNestedResult) {
          LOG.debug("Fetching {} action's nested-dump file chunks for task={}, dumpfileid={}",
              task.getSubject().getClass().getSimpleName(), task.getId(), dumpFileId);
          return task.getSubject().getNestedDumpFileChunks(task.getId(), dumpFileId).getItem();
        } else {
          LOG.debug("Fetching {} action's dump file chunks for task={}", task.getSubject().getClass().getSimpleName(),
              task.getId());
