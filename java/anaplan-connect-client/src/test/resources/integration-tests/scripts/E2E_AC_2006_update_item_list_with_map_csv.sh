...
. ./config/config.sh
FilePath="data/INTEGRA-3055-mapped.csv"
FileMapPath="data/INTEGRA-3055-map.properties"
ListId="101000000138"

Operation="-debug -service ${ServiceUrl} -auth ${AuthUrl} -workspace ${WorkspaceId} -model ${ModelId} -l ${ListId} -itemmappingproperty ${FileMapPath} -execute -updateItems:csv ${FilePath} "
