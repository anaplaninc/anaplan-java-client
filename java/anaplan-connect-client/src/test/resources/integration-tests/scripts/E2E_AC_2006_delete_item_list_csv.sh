...
. ./config/config.sh
FilePath="data/INTEGRA-3055.csv"
ListId="101000000138"
OutputLog="ItemsLog.txt"

Operation="-debug -service ${ServiceUrl} -auth ${AuthUrl} -workspace ${WorkspaceId} -model ${ModelId} -l ${ListId} -execute -deleteItems:csv ${FilePath} -output:json '${OutputLog}'"
