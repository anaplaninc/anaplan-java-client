...
ActionName=""
ImportName="cyrillicImport"
ExportName=""
FileName="cyrillicSource.csv"
FilePath=""
ChunkSize=20
OutputName=""
ErrorDump="dump/Cyrillic_Errors.txt"

Operation="-debug -service ${ServiceUrl} -auth ${AuthUrl} -workspace ${WorkspaceId} -model ${ModelId}  -file '${FileName}' -jdbcproperties 'cyrillicTest.properties' -chunksize ${ChunkSize} -import '${ImportName}' -execute -output '${ErrorDump}' "
