...
FileName1="export/basic-auth-process-coresrv-2779-module - multicol.csv"
FileName2="export/basic-auth-process-coresrv-2775-module - tabularSingleCol.csv"

Operation="-debug -service ${ServiceUrl} -auth ${AuthUrl} -workspace ${WorkspaceId} -model ${ModelId} -file '${ImportDataSource1}' -put '${ImportFileName1}' -file '${ImportDataSource2}' -put '${ImportFileName2}'  -process '${ProcessName}' -execute -file '${ExportName1}' -get '${FileName1}' -file '${ExportName2}'  -get '${FileName2}' -output '${ErrorDump}'"
