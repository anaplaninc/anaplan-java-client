...
ExportName=""
FileName=""
FilePath="list-data/list-items-includeall.json"
ChunkSize=1
OutputName=""
ErrorDump=""
ListName="non existingList"

Operation="-debug -service ${ServiceUrl} -auth ${AuthUrl} -workspace '${WorkspaceName}' -model '${ModelName}' -list '${ListName}' -execute:all -get:json '${FilePath}'"
